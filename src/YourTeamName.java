 ...
 * 14317761 - Marcin Checinski
 * 14337341 - Damian Kluziak
 * 14366596 - Crischelle Pana
 */
...
//		System.out.println();
		...
		if (possibleCountriesToReinforce.isEmpty()) {
			...
			//3.)	Third Priority
				...
				//else if you don't have a country in the continent that has the biggest ratio
				else {
					//System.out.println("HOPEFULLY IT'LL NEVER GO IN THIS ELSE #2");
				}
		...
		for (n = 0; n < possibleCountriesToReinforce.size(); n++) {
			...
			int  randomNum = rand.nextInt(possibleCountriesToReinforce.size()) + 0;
//			System.out.println(">random number: " + randomNum);			
			//System.out.println("PossibleCountryToReinforce #11");
			command = GameData.COUNTRY_NAMES[possibleCountriesToReinforce.get(randomNum)];
			command = command.replaceAll("\\s", "");
			if (noToReinforce > 3) {
				noToReinforce /= 2;
				command += " " + noToReinforce;
		...
		for (n = 0; n < ownedCountries.size(); n++) {
			if (ownedCountries.get(n) == 31 || ownedCountries.get(n) == 32 || ownedCountries.get(n) == 34 || ownedCountries.get(n) == 37 || 
					ownedCountries.get(n) == 8 || ownedCountries.get(n) == 4 || ownedCountries.get(n) == 7 || ownedCountries.get(n) == 22 || ownedCountries.get(n) == 23 || ownedCountries.get(n) == 14) {
				//System.out.println("PossibleCountryToReinforce #12");
				command = GameData.COUNTRY_NAMES[ownedCountries.get(n)];
				command = command.replaceAll("\\s", "");
				if (noToReinforce > 3) {
					noToReinforce /= 2;
		...
		for (n = 0; n < ownedCountries.size(); n++) {
			if (GameData.CONTINENT_IDS[ownedCountries.get(n)] == biggestRatioContinent) {
				if (board.getNumUnits(ownedCountries.get(n)) <= reinforcementLimit) {
					//System.out.println("PossibleCountryToReinforce #13");
					command = GameData.COUNTRY_NAMES[ownedCountries.get(n)];
					command = command.replaceAll("\\s", "");
					if (noToReinforce > 3) {
						noToReinforce /= 2;
		...
		for (n = 0; n < ownedCountries.size(); n++) {
			if (GameData.CONTINENT_IDS[ownedCountries.get(n)] == biggestRatioContinent) {
				//System.out.println("PossibleCountryToReinforce #14");
				command = GameData.COUNTRY_NAMES[ownedCountries.get(n)];
				command = command.replaceAll("\\s", "");
				if (noToReinforce > 3) {
					noToReinforce /= 2;
		...
		for (n = 0; n < ownedCountries.size(); n++) {
			if (board.getNumUnits(ownedCountries.get(n)) <= reinforcementLimit) {
				//System.out.println("PossibleCountryToReinforce #15");
				command = GameData.COUNTRY_NAMES[ownedCountries.get(n)];
				command = command.replaceAll("\\s", "");
				if (noToReinforce > 3) {
					noToReinforce /= 2;
		...
		//debugging reasons		
//		Random rand = new Random();
//		int  randomNum = rand.nextInt(playersCountries.length) + 0;
//		System.out.println(">random number: " + randomNum);
//		
//		command = GameData.COUNTRY_NAMES[playersCountries[randomNum]];
//		command = command.replaceAll("\\s", "");
//		command += " 3";
//		
//		command = GameData.COUNTRY_NAMES[playersCountries[0]];
//		command = command.replaceAll("\\s", "");
//		command += " 3";
	...
	public String getPlacement (int forPlayer) {
		...
		int[] countriesOwned = new int[counter];
		
		//assigns owned countries to an array
		j = 0;
		for (i = 0; i < GameData.NUM_COUNTRIES; i++) {
			if (board.getOccupier(i) == forPlayer){
				countriesOwned[j] = i;
		...
		//Counts number of enemy adj countries to countries this neutral own
		int[] noOfEnemAdjCountOfOwn = new int[countriesOwned.length];
		int noOfAdjEnem = 0, p = 0, r = 0;

		i = 0;
		for (j = 0; j < countriesOwned.length; j++) {
			p = 0;
			while (p < GameData.ADJACENT.length) {
				if (countriesOwned[j] == p) {
					for (r = 0; r < GameData.ADJACENT[p].length; r++){
						if (board.getOccupier(GameData.ADJACENT[p][r]) == playerOne) {
							noOfAdjEnem++;
						}
					}
					noOfEnemAdjCountOfOwn[i] = noOfAdjEnem;
					noOfAdjEnem = 0;
					i++;
				}
				p++;
		...
		//Counts number of own adj countries to countries this neutral own country
		int[] noOfOwnAdjCountOfOwn = new int[countriesOwned.length];
		int noOfAdjOwn = 0;
		...
		for (j = 0; j < countriesOwned.length; j++) {
			...
			while (p < GameData.ADJACENT.length) {
				if (countriesOwned[j] == p) {
					for (r = 0; r < GameData.ADJACENT[p].length; r++){
						if (board.getOccupier(GameData.ADJACENT[p][r]) == myID) {
							noOfAdjOwn++;
						}
					}
					noOfOwnAdjCountOfOwn[i] = noOfAdjOwn;
					noOfAdjOwn = 0;
					i++;
				}
				p++;
		...
		//if any of our countries is fully surrounded or equal or more than half by enemy then reinforce on taht.
		for (i = 0; i < countriesOwned.length; i++) {
			if (noOfEnemAdjCountOfOwn[i] > (GameData.ADJACENT[countriesOwned[i]].length/2) || noOfEnemAdjCountOfOwn[i] > noOfOwnAdjCountOfOwn[i]) {
		...
		for (i = 0; i < countriesOwned.length; i++) {
			if (noOfEnemAdjCountOfOwn[i] == noOfOwnAdjCountOfOwn[i] || noOfOwnAdjCountOfOwn[i] >= (GameData.ADJACENT[countriesOwned[i]].length/2)) {
				//do not do anything.
			}
		}
		
		//find the enemy country that holds the lowest army units
		//and check if it's adjacent to any of the neutral's armies then reinforce that country
		for (i = 0; i < countriesOwnedbyEnemy.length; i++) {
			for (j = i; j < countriesOwnedbyEnemy.length; j++) {
				if (board.getNumUnits(countriesOwnedbyEnemy[i]) < board.getNumUnits(countriesOwnedbyEnemy[j])) {
		...
		for (i = 0; i < countriesOwned.length ; i++) { 
			j = 0;
			while (j < countriesOwnedbyEnemy.length) {
				if (board.isAdjacent(countriesOwned[i], countriesOwnedbyEnemy[j])) {
					command = GameData.COUNTRY_NAMES[countriesOwned[i]];
					command = command.replaceAll("\\s", "");
					return(command);
				}
		...
		//debugging reasons
//		Random rand = new Random();
//		int  randomNum = rand.nextInt(countriesOwned.length) + 0;
//		System.out.println(">random number: " + randomNum);
//		command = GameData.COUNTRY_NAMES[countriesOwned[randomNum]];
		
//		System.out.println(">command: " + command);
//		command = GameData.COUNTRY_NAMES[countriesOwned[0]];
...
//			if (borderingCountriesOwned != GameData.ADJACENT[playersCountries[i]].length) {
//				fortifees++;
//			}
		}
